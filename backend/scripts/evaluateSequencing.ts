...
interface BenchmarkResult {
  ...
  meanArousalDelta: number;
  maxArousalDelta: number;
  jarringTransitionsCount: number; // Arousal jump > 0.35
  smoothnessScore: number; // 100 - (meanArousalDelta * 100)
}

function computeMetrics(engineName: string, tracks: MusavTrack[], rejectedCount: number): BenchmarkResult {
  if (tracks.length < 2) {
    return {
      engineName,
...
function renderTrajectoryMap(tracks: MusavTrack[]): string {
  if (tracks.length === 0) return 'Empty sequence';
...
function runBenchmark() {
  ...
  const trackMap = new Map<string, MusavTrack>();
  ...
  const driftResult = generateDriftPlaylist(driftTracks);
  const driftSequencedMusavTracks: MusavTrack[] = driftResult.tracks
    .map(dt => trackMap.get(dt.videoId))
    .filter((t): t is MusavTrack => t !== undefined);

  const tuneItMetrics = computeMetrics(
    'TuneIt Mental Drift Engine',
    driftSequencedMusavTracks,
    driftResult.harshTracks.length
  );
