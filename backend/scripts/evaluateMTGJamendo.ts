...
import { processRiseAlgorithm } from '../src/utils/riseAlgorithm';
...
function evaluateRiseOnPool(pool: FrameTrack[], engineLabel: string): ScaleResult {
  ...
  const start = performance.now();
  const output = processRiseAlgorithm(riseInputs);
  const end = performance.now();
...
function runMTGJamendoEvaluation() {
  ...
  const riseScaleResults: ScaleResult[] = [];
  for (const scale of SCALES) {
    const pool = generateScaledPool(baseTracks, scale);
