...
function renderAsciiTrajectoryMap(sequenced) {
    ...
    return sequenced
        ...
        if (track.role === 'SETUP')
            roleTag = '[SETUP]   ';
        else if (track.role === 'CURVEBALL')
            roleTag = '⚡[CURVEBALL!]';
        else if (track.role === 'RECOVERY')
            roleTag = '[RECOVER] ';
...
function runUnhingedEvaluation() {
    ...
    console.log('========================================================================================\n');
    const mtgPath = path_1.default.join(__dirname, '..', 'tests', 'fixtures', 'mtg_jamendo_dataset.json');
    const musavPath = path_1.default.join(__dirname, '..', 'tests', 'fixtures', 'musav_30.json');
    if (!fs_1.default.existsSync(mtgPath) || !fs_1.default.existsSync(musavPath)) {
        console.error('❌ Benchmark dataset fixtures not found in /tests/fixtures/');
        process.exit(1);
    }
    const jamendoTracks = JSON.parse(fs_1.default.readFileSync(mtgPath, 'utf-8'));
    const musavTracks = JSON.parse(fs_1.default.readFileSync(musavPath, 'utf-8'));
    ...
    // Run Unhinged Engine
    const unhingedOutput = (0, unhingedAlgorithm_1.processUnhingedAlgorithm)(combinedPool);
    ...
    console.log(` ${check1YieldMet ? '✅' : '❌'} Check 1: Yield Retention Target (80.0% - 92.0%, Ideal 85%-90%) -> Actual Yield = ${retentionVal.toFixed(1)}% (${sequencedTracks.length}/${combinedPool.length})`);
    console.log(` ${check2AnchoredMet ? '✅' : '❌'} Check 2: 100% Curveball Transition Anchors Verified -> ${curveballTracks.length - unanchoredCurveballs.length}/${curveballTracks.length} Curveballs Strictly Anchored`);
    console.log(` ${check3BalancedAnchorsMet ? '✅' : '❌'} Check 3: Dual High-Coverage Anchor Rebalance (Harmonic & Freq/Rhythm >= 60%) -> Harmonic = ${metrics.harmonicAnchorRate}, Freq/Rhythm = ${metrics.frequencyAnchorRate}`);
    console.log('\n========================================================================================');
