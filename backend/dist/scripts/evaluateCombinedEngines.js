...
function renderTrajectoryMap(title, tracks, getEnergy) {
    if (tracks.length === 0)
        return `${title}\n  (No tracks output)\n`;
...
function computeMetrics(tracks, getEnergy, rejectedCount) {
    if (tracks.length < 2) {
        return {
...
function runCombinedEvaluation() {
    ...
    const defaultPath = path_1.default.join(__dirname, 'customUserPlaylist.json');
    const fallbackPath = path_1.default.join(__dirname, 'musavBenchmarkData.json');
    let dataPath = defaultPath;
    if (argPath && fs_1.default.existsSync(argPath)) {
        dataPath = argPath;
    }
    else if (!fs_1.default.existsSync(defaultPath)) {
        dataPath = fallbackPath;
    }
    console.log(`📂 Loading input track dataset from: ${path_1.default.basename(dataPath)}`);
    ...
    const driftInputs = rawData.map((t, idx) => ({
    ...
    const driftOutput = (0, driftAlgorithm_1.generateDriftPlaylist)(driftInputs);
    const driftMetrics = computeMetrics(driftOutput.tracks, t => t.intensityScore, driftOutput.harshTracks.length);
    // 2. FRAME ENGINE
    const frameInputs = rawData.map((t, idx) => ({
    ...
    const frameOutput = (0, frameAlgorithm_1.processFrameEngine)(frameInputs);
    ...
    // 3. UNHINGED ENGINE
    const unhingedInputs = rawData.map((t, idx) => ({
        id: t.id || t.videoId || `track_${idx + 1}`,
        title: t.title,
        artist: t.artist,
        bpm: t.bpm || t.estimatedBpm || 120,
    ...
    const unhingedOutput = (0, unhingedAlgorithm_1.processUnhingedAlgorithm)(unhingedInputs);
    ...
    // 4. RISE ALGORITHM
    const riseInputs = rawData.map((t, idx) => ({
        id: t.id || t.videoId || `track_${idx + 1}`,
        title: t.title,
        artist: t.artist,
        bpm: t.bpm || t.estimatedBpm || 120,
    ...
    const riseOutput = (0, riseAlgorithm_1.processRiseAlgorithm)(riseInputs);
