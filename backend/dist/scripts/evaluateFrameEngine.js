...
function computeMetrics(engineName, tracks, rejectedCount) {
    ...
    for (let i = 0; i < tracks.length - 1; i++) {
        const current = (0, frameAlgorithm_1.getTrackEnergy)(tracks[i]);
        const next = (0, frameAlgorithm_1.getTrackEnergy)(tracks[i + 1]);
...
function renderAsciiTrajectoryMap(acceptedTracks) {
    ...
    return acceptedTracks
        ...
        const energy = (0, frameAlgorithm_1.getTrackEnergy)(track);
...
function runFrameEngineEvaluation() {
    ...
    console.log('========================================================================\n');
    const fixturePath = path_1.default.join(__dirname, '..', 'tests', 'fixtures', 'musav_30.json');
    if (!fs_1.default.existsSync(fixturePath)) {
        console.error(`❌ Error: Benchmark dataset fixture not found at ${fixturePath}`);
        process.exit(1);
    }
    const rawTracks = JSON.parse(fs_1.default.readFileSync(fixturePath, 'utf-8'));
    ...
    const output = (0, frameAlgorithm_1.processFrameAlgorithm)(rawTracks);
    const frameMetrics = computeMetrics('TuneIt Frame Engine (Cinematic)', output.acceptedTracks, output.rejectedTracks.length);
    ...
    if (output.rejectedTracks.length > 0) {
        ...
        output.rejectedTracks.forEach((item, i) => {
            const energy = (0, frameAlgorithm_1.getTrackEnergy)(item.track);
            console.log(`  ❌ Rejected: "${item.track.artist} - ${item.track.title}" | BPM: ${item.track.bpm} | Energy: ${energy.toFixed(2)} -> Reason: ${item.reason}`);
        });
    }
    ...
    for (let i = 0; i < act3Tracks.length - 1; i++) {
        const e1 = (0, frameAlgorithm_1.getTrackEnergy)(act3Tracks[i]);
        const e2 = (0, frameAlgorithm_1.getTrackEnergy)(act3Tracks[i + 1]);
        if (e2 > e1 + 0.0001) {
