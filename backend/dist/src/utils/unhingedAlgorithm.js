...
exports.isCamelotCompatible = isCamelotCompatible;
exports.checkAnchor = checkAnchor;
exports.calculatePolarDistance = calculatePolarDistance;
exports.calculateEnergyDelta = calculateEnergyDelta;
exports.processUnhingedAlgorithm = processUnhingedAlgorithm;
/**
...
function deriveCamelotKey(id, bpm, arousal) {
    let hash = 0;
...
function deriveSubBassDensity(id, bpm, arousal) {
    let hash = 0;
    for (let i = 0; i < id.length; i++) {
...
function deriveGenreFromTrack(title, artist, bpm, arousal) {
    if (bpm < 90 && arousal < 0.4)
        return 'Ambient / Chill';
    if (bpm < 110 && arousal < 0.5)
        return 'Downtempo / Jazz';
    if (bpm >= 110 && bpm <= 130 && arousal >= 0.5)
        return 'Indie Electronic';
    if (bpm > 130 && arousal >= 0.7)
        return 'Hyper Energy / Rock';
    if (arousal >= 0.7)
        return 'Electro / Dance';
    return 'Alternative';
}
function isCamelotCompatible(keyA, keyB) {
    if (!keyA || !keyB)
        return false;
 ...
 */
function checkAnchor(prev, cand) {
    ...
    const isBpmSync = Math.abs(bpmRatio - 1.0) <= 0.08 ||
    ...
    let anchorType = 'NONE';
    if (harmonicMatched && frequencyRhythmMatched)
        anchorType = 'BOTH';
    else if (harmonicMatched)
        anchorType = 'HARMONIC';
    else if (frequencyRhythmMatched)
        anchorType = 'FREQUENCY_RHYTHM';
...
function calculatePolarDistance(a, b) {
...
function calculateEnergyDelta(a, b) {
    const energyA = a.arousal ?? a.intensity ?? 0;
    const energyB = b.arousal ?? b.intensity ?? 0;
 ...
 */
function processUnhingedAlgorithm(inputPool) {
    ...
    validCandidates.forEach(cand => {
        ...
        const harmonicPartners = validCandidates.filter(other => other.id !== cand.id && isCamelotCompatible(cand.key, other.key));
        const hasStrongOppositeAnchor = harmonicPartners.some(other => Math.abs(other.arousal - cand.arousal) >= 0.40 && Math.abs(cand.subBassDensity - other.subBassDensity) <= 0.20);
    ...
    const pickSmoothSetupTrack = (prevTrack) => {
        let bestIdx = 0;
