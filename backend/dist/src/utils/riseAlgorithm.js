...
exports.processRiseEngine = void 0;
exports.getTrackEnergy = getTrackEnergy;
exports.normalizeRiseTrack = normalizeRiseTrack;
exports.isCamelotCompatible = isCamelotCompatible;
exports.computeMedian = computeMedian;
exports.processRiseAlgorithm = processRiseAlgorithm;
exports.calculateRiseMetrics = calculateRiseMetrics;
exports.checkEveryWindowHasDip = checkEveryWindowHasDip;
/**
 ...
 */
function normalizeRiseTrack(raw, index = 0) {
 ...
 */
function processRiseAlgorithm(inputPool) {
...
exports.processRiseEngine = processRiseAlgorithm;
/**
 * Arranges Act II tracks to prioritize Camelot key compatibility with small energy deltas.
 */
function sortAct2WithHarmonics(bucket, prevTrack) {
    if (bucket.length <= 1)
        return bucket;
    const result = [];
 ...
 */
function applyStaircasePattern(items) {
 ...
 */
function calculateRiseMetrics(tracks) {
    ...
    for (let i = 0; i < n - 1; i++) {
        ...
        if (absDelta > maxDeltaEnergy)
            maxDeltaEnergy = absDelta;
        if (absDelta > 0.35)
            jarringCount++;
        if (delta < 0)
            negativeDeltaCount++;
    ...
    for (let i = 0; i < n - 1; i++) {
        ...
        if (delta < 0) {
            ...
            streak++;
            if (streak > maxWindowWithoutDip)
                maxWindowWithoutDip = streak;
        }
    }
    // 4. Segment Medians
    const segments = {
 ...
 */
function checkEveryWindowHasDip(tracks, windowSize = 6) {
    if (tracks.length < windowSize)
        return true;
