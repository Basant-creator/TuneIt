...
export interface Track {
  ...
  artist: string;
  bpm: number;
  intensity: number; // Primary energy feature metric (0.0 to 1.0)
  valence: number;
  arousal?: number;  // Optional alias for intensity
}

export type ActType = 'ACT_I' | 'ACT_II' | 'ACT_III';

export type ActDirection = 'SPIKE_AND_TANK' | 'TANK_AND_SPIKE';

export interface FrameMetrics {
  baselineIntensity: number; // μ (mean intensity)
  baselineArousal?: number;  // Alias for baselineIntensity
...
export interface ActAllocation {
  act1: number; // 30%
  act2: number; // 45%
  act3: number; // 25%
}

export interface AcceptedTrack extends Track {
  act: ActType;
}
 ...
 */
export function getTrackEnergy(t: Track): number {
 ...
 */
export function computeSkewness(values: number[], mean: number, stdDev: number): number {
 ...
 */
function isReachableFromBaseline(
  target: Track,
  pool: Track[],
  baseline: number,
  maxStep: number = 0.28
  ...
  if (Math.abs(targetVal - baseline) <= maxStep) return true;
 ...
 */
export function computeSmoothnessScore(tracks: Track[]): {
  meanDelta: number;
  maxDelta: number;
  jarringJumps: number;
  score: number;
...
export function processFrameAlgorithm(inputTracks: Track[]): FrameOutput {
  ...
  let fullRaw: Track[] = [...act1Tracks, ...act2Pool, ...act3Tracks];

  // Re-assign act markers based on track index boundaries
  const finalAct1: AcceptedTrack[] = fullRaw.slice(0, act1Count).map(t => ({ ...t, act: 'ACT_I' as ActType }));
  const finalAct2: AcceptedTrack[] = fullRaw.slice(act1Count, act1Count + act2Count).map(t => ({ ...t, act: 'ACT_II' as ActType }));
  const finalAct3: AcceptedTrack[] = fullRaw.slice(act1Count + act2Count).map(t => ({ ...t, act: 'ACT_III' as ActType }));

  const acceptedTracks: AcceptedTrack[] = [...finalAct1, ...finalAct2, ...finalAct3];
...
export const processFrameEngine = processFrameAlgorithm;
